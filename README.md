...
- **编辑与添加**：可改单词本信息、改 / 删单个词条；「添加词」支持粘贴文本 AI 提取追加或手动单条，
  自动按英文去重（同词不重复入库），新词同步进 SRS 复习池
- **一键去重**：单词本内「去重」按钮可清理历史已有的重复英文词，每个词只保留学习进度最高的一条
...
- **备份与还原**：把单词本 / 学习进度 / 翻译记录 / 问答会话 / 设置导出为一个文件，换机后导入合并恢复；
  平台中立格式，**安卓 ↔ iPhone 可跨平台互导**（模型文件除外，换机重下）
