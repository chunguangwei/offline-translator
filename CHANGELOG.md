...
### 修复
- **Android 单词本导入 RTF 自动提纯**：导入从备忘录 / Pages 导出的 RTF 文件时，
  不再出现满屏排版标记，自动提取出纯文字（含中文）。此前仅 iOS 已修，本版对齐 Android
...
### 新增
- **「学习」全面升级为间隔重复（SRS）记忆系统**：跨全部单词本 + 生词本聚合
  「🔥 连续打卡 · 今日到期 N 张 · 开始复习」。记住的词隔 1/3/7/16/30 天逐级再考、
  到顶即掌握；没记牢的很快回来。彻底告别"同一组词原地重复"
- **错题本**：反复答错的词单独汇总强化
- **每日提醒**：到点本地通知提醒复习（纯本地、不联网，默认关）
- 单词本「今日复习」改走 SRS；「全量抽查」保留为不计进度的纯练手

### 修复
- 单词本导入：支持 .txt 与 RTF（自动提取纯文字，不再出现满屏排版标记）；
  长文本时「提取/保存」按钮始终可见；输入键盘可下滑/点「完成」收起
- 修复翻卡评分后偶发跳回第一张、点选时闪过其它词
...
### 修复
- **切换 / 删除模型更稳**：修复正在翻译或问答时切换模型可能导致的闪退
- **翻译更准**：补强约束，避免极少数情况下把译文翻成第三种语言或给出多版本
- **语音转写更忠实**：转写采样进一步降温，减少"听成别的话"的情况
- **单词本提取更准**：上传生词本时提取改用低温，减少乱编 / 漏词
- **问答页语音可选中英方向**：中文界面下也能直接转写英文语音（与翻译页一致）

### 安全与隐私
- 应用内更新增加 APK 哈希强校验 + 官方下载地址校验，进一步保证升级包未被篡改
- 隐私说明据实更新：如实披露启动时会联网获取启动图与更新信息（均不含任何个人数据）
...
### 修复
- **录音波形不随声音动**：部分机型（vivo 实测）麦克风采样存在直流偏置，
  幅度计算先去均值再统计，波形恢复跟手
- **语音转写混入提示词样内容**（iOS）：转写消息体改为指令在前、音频在后
- iOS 翻译页补「清空」按钮（输入非空时显示）
- 问答转写等待文案「即将发送…」→「整理文字中…」（转写结果是回填输入框
  由用户确认，旧文案为自动发送时代残留，易误导）

### 备注
- 排查中确认：插 USB 连电脑时部分 vivo 机型会把麦克风路由到 USB 设备导致
  收音为静音——属系统行为非 App 问题，拔线即恢复
...
### 新增（双端同步）
- **问答角色预设**：默认助手 / E人陪聊（中文来中文回、英文来英文回的外向搭子）/
  翻译官 / 语法老师 / 写作润色 / 英语陪练（英语闲聊 + ✏️ 温和纠错），
  顶栏面具图标一键切换（持久化）
- **拍照 / 选图翻译**：翻译页新增相机与相册按钮，识别图中文字逐行
  「原文 => 译文」对照，结果流式输出
- **生词本 + 单词练习**：历史页可收藏翻译记录为生词，抽卡练习
  （点卡翻面看译文，认识 / 再练循环）
- **译文分享卡片**：暖色品牌渐变卡图一键分享（微信 / 保存相册等）
- 启动页彩蛋：点 logo 角色会弹跳 + 连眨眼
- **「学习」独立 Tab**：底栏五分区（翻译/问答/学习/历史/设置），生词本练习与
  单词本统一入口；历史回归纯翻译记录
- **单词本系统**：上传生词文本（粘贴或选 txt），AI 离线提取「英文 => 释义 => 注释」
  建库（纯英文词表自动补释义）；自定名称/用途/每日学习量；
  今日学习（未掌握词取每日量）与全量随机抽查；中→英 / 英→中 / 混合翻卡测试，
  「认识」3 次即掌握、「不认识」归零放回队尾

### 修复
- 语音转写忠实度：超低温采样 + 强化逐字转写指令（此前偶发意译/接话而非原样转写）
- 翻译/拍照翻译低温采样（防输出多种语言）
- 切换问答角色自动开新会话（防旧角色语气带偏）
- 单词本/练习弹窗改实底卡片（不再透出下层内容）
- 翻译页录音启动竞态修复 + 录音器加初始化校验与 MIC 音源回退

### 已知问题
- 部分 vivo 机型翻译页语音输入仍可能提示拿不到麦克风（排查中，问答页语音正常）

---

## [iOS 1.0.0] — 2026-06-11

> iOS 端从零起盘到与 Android 功能完全对齐，真机（iPhone 13）验收通过。
> 分发：现阶段开发签名（Xcode ⌘R），App Store 上架准备中。

### 新增
- **四 Tab 全量**：翻译（文字 + 语音输入）/ 问答（多轮会话、上下文压缩、
  图片问答 + 追问重喂、语音输入、全屏图预览）/ 历史（翻译记录去重）/
  设置（主题、应用语言、推理后端、模型源四档、关于与许可）
- **品牌启动页**：眨眼 logo 角色 + 轮换标语，6 秒可跳过；splash.json 远程换图（与 Android 同契约）
- **模型下载**：ModelScope 国内首选 + 多源兜底；background URLSession
  （锁屏 / 杀进程不中断，系统断点续传）；开发期可选"模型随包"
- App 图标（与 Android 同款品牌视觉，1024 全出血无 alpha）
- 推理引擎：LiteRT-LM 官方 Swift 封装，原生多轮会话 API，
  视觉 / 音频编码器 CPU、主模型 GPU→CPU 回退；4GB 设备内存上限 entitlement
- 键盘点空白 / 下滑收起
...
### 修复
- **检查更新不再报 403**：检查更新改读仓库内静态清单（gh-proxy / jsDelivr / raw 多源，
  国内可达），不再依赖 api.github.com（未认证限流 60 次/小时/IP，部分网络直接 403），
  API 仅作最后兜底
- 更新 APK 下载新增 gh-proxy 加速候选，github.com 直连兜底
- 更新弹窗内容改为人话版变更说明（不再是一堆链接）
...
### 修复
- **模型下载大提速**：新增 ModelScope（魔搭/阿里云 OSS）为国内首选下载源。
  此前的 hf-mirror 对 HF Xet 仓库只会 308 重定向回被墙的 huggingface.co，
  没有实际加速效果（"下载太慢"的根因）。顺序：ModelScope → hf-mirror → 官方。
